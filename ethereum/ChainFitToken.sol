...
contract ChainFit {
    ...
    function addGymVisit(address _user, uint _visitTime, string memory _hash) public {
        GymVisit memory g = GymVisit(gymVisitsCount, _user, _visitTime, _hash, Result.InProgress, new uint[](0));
        gymVisits[gymVisitsCount] = g;
        gymVisitsCount++;
    }

    function addGymVisitRate(uint visitId, address userRated, uint ratingTime, Rate rate, RateSource rateSource) public {
        GymVisitRate memory r = GymVisitRate(visitId, userRated, ratingTime, rate, rateSource);
        gymVisitRates[gymVisitRatesCount] = r;
    ...
    function checkVisit(uint visitId) external returns(Result result){
    ...
    function getVisits(address user, uint historyTime) view internal notZero(historyTime) returns(GymVisit[] memory visits){
        if(gymVisitsCount == 0) return new GymVisit[](0);
        ...
        uint indexRet = 0;
        for(uint i=gymVisitsCount-1; i>=0; i--){
            if(gymVisits[i].visitTime >= minTime){
                if(gymVisits[i].user == user){
                    if(indexRet < retLen) ret[indexRet++] = gymVisits[i];
                    else break;
                }
            }
            else break;
    ...
    // Get all the visits for user
    function getVisits(address user) view public returns(GymVisit[] memory visits){
        return getVisits(user, block.timestamp - Const.APP_START_DATE);
    }
